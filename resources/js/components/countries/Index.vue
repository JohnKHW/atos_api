<template>
    <div>
        ...
        <b-button variant="success" to="/admin/countries/create">
...
export default {
    ...
    methods: {
        edit(item) {
            window.location.href = `/admin/countries/${item.id}/edit`;
        },
        deleteItem(item) {
            axios.delete(`/admin/countries/${item.id}`);
        },
        getResults(page = 1) {
            window.location.href = `/admin/countries?page=${page}`;
		},
    },
}
</script>