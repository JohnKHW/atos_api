<template>
    <div>
        <div align="right">
            ...
            <b-button variant="success" to="/admin/supermarkets/create">
...
export default {
    ...
    methods: {
        edit(item) {
            window.location.href = `/admin/supermarkets/${item.id}/edit`;
        },
        deleteItem(item) {
            axios.delete(`/admin/supermarkets/${item.id}`);
        },
        getResults(page = 1) {
            window.location.href = `/admin/supermarkets?page=${page}`;
		},
    },
}
</script>